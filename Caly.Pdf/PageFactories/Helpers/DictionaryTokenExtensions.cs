...
namespace Caly.Pdf.PageFactories.Helpers
{
    // From PdfPig DictionaryTokenExtensions
    internal static class DictionaryTokenExtensions
    {
        ...
        {
            result = default;
            if (token.TryGet(name, out var appearancesToken) && DirectObjectFinderCaly.TryGet(appearancesToken, scanner, out T innerResult))
            {
                result = innerResult;
                return true;
            }
    ...
    internal static class ArrayTokenExtensions
    {
        ...
        public static PdfRectangle ToRectangle(this ArrayToken array, IPdfTokenScanner tokenScanner)
        {
            ...
            return new PdfRectangle(DirectObjectFinderCaly.Get<NumericToken>(array[0], tokenScanner).Double,
                DirectObjectFinderCaly.Get<NumericToken>(array[1], tokenScanner).Double,
                DirectObjectFinderCaly.Get<NumericToken>(array[2], tokenScanner).Double,
                DirectObjectFinderCaly.Get<NumericToken>(array[3], tokenScanner).Double);
        }
        ...
        public static PdfRectangle ToIntRectangle(this ArrayToken array, IPdfTokenScanner tokenScanner)
        {
            ...
            return new PdfRectangle(DirectObjectFinderCaly.Get<NumericToken>(array[0], tokenScanner).Int,
                DirectObjectFinderCaly.Get<NumericToken>(array[1], tokenScanner).Int,
                DirectObjectFinderCaly.Get<NumericToken>(array[2], tokenScanner).Int,
                DirectObjectFinderCaly.Get<NumericToken>(array[3], tokenScanner).Int);
        }
    }
}
