...
namespace Caly.Pdf.Layout
{
    ...
    /// </summary>
    public sealed class CalyDocstrum //: IPageSegmenter
    {
        ...
        public IReadOnlyList<PdfTextBlock> GetBlocks(IEnumerable<PdfWord> words)
        {
            if (words?.Any() != true)
            {
        ...
        {
            words = words.Where(w => !w.Letters.All(l => l.Value.Span.IsWhiteSpace())).ToArray();
        ...
        public static IEnumerable<PdfTextLine> GetLines(IReadOnlyList<PdfWord> words, double maxWLDistance, AngleBounds wlBounds,
            string wordSeparator, int maxDegreeOfParallelism)
        {
            ...
            {
                yield return new PdfTextLine(gr.OrderByReadingOrder().ToArray(), default, 0, 0, 0);
            }
        }
            ...
            double maxBLDistance, AngleBounds angularDifferenceBounds, double epsilon, string lineSeparator, int maxDegreeOfParallelism)
        {
            ...
            {
                yield return new PdfTextBlock(g.OrderByReadingOrder().ToArray(), default, 0, 0, 0);
            }
        }
