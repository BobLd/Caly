    ...
    internal sealed class PdfPigPdfService : IPdfService
    {
        ...
        public async Task<int> OpenDocument(IStorageFile? storageFile, string? password, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                return await Task.Run(() =>
                {
                    ...
                    document.AddPageFactory<PdfPageInformation, PageInformationOptimisedFactory>();
        ...
        public async Task<PdfPageInformation?> GetPageInformationAsync(int pageNumber, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                await File.WriteAllTextAsync($"error_avalonia_page_information_{Guid.NewGuid()}.txt",
                    ex.ToString());
                throw;
            }
            finally
            {
                if (semaphore.CurrentCount == 0 && !isDiposed()) // !cancellationToken.IsCancellationRequested &&
                {
                    semaphore.Release();
                }
            }
        ...
        public async Task<IRef<SKPicture>?> GetRenderPageAsync(int pageNumber, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                await File.WriteAllTextAsync($"error_avalonia_render_page_{Guid.NewGuid()}.txt", ex.ToString());
                throw;
            }
            finally
            {
            ...
            //using (var serialized = pic.Serialize())
            //using (var fs = new FileStream("serialized_cpu.skp", FileMode.Create))
            //{
            //    serialized.SaveTo(fs);
            //}
        ...
        public async Task<PdfTextLayer?> GetTextLayerAsync(int pageNumber, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (cancellationToken.IsCancellationRequested || document is null || isDiposed())
                {
                    return null;
                }
                ...
                return PdfTextLayerHelper.GetTextLayer(page);
            ...
            catch (Exception ex)
            {
                ...
                await File.WriteAllTextAsync($"error_avalonia_text_layer_{Guid.NewGuid()}.txt", ex.ToString());
                throw;
            }
            finally
            {
        ...
        public async Task<ObservableCollection<PdfBookmarkNode>?> GetPdfBookmark(CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                await File.WriteAllTextAsync($"error_avalonia_bookmark_{Guid.NewGuid()}.txt", ex.ToString());
                throw;
            }
            finally
            {
