...
namespace Caly.Core.Controls
{
    /*
     * MIT licence
     * This is a copy of https://github.com/AvaloniaUI/Avalonia/blob/master/src/Avalonia.Controls/LayoutTransformControl.cs
     *
     * We need our own version of LayoutTransformControl in order to increase the DecimalsAfterRound property from 4 to 8.
     *
     * The initial value results in issues when zooming in and out at the end of VERY large pdf documents.
     */

    /// <summary>
    /// Control that implements support for transformations as if applied by LayoutTransform.
    /// </summary>
    public class CalyLayoutTransformControl : Decorator
    {
        public static readonly StyledProperty<ITransform?> LayoutTransformProperty =
            AvaloniaProperty.Register<CalyLayoutTransformControl, ITransform?>(nameof(LayoutTransform));

        public static readonly StyledProperty<bool> UseRenderTransformProperty =
            AvaloniaProperty.Register<CalyLayoutTransformControl, bool>(nameof(UseRenderTransform));

        static CalyLayoutTransformControl()
        {
            ClipToBoundsProperty.OverrideDefaultValue<CalyLayoutTransformControl>(true);

            LayoutTransformProperty.Changed
                .AddClassHandler<CalyLayoutTransformControl>((x, e) => x.OnLayoutTransformChanged(e));

            ChildProperty.Changed
                .AddClassHandler<CalyLayoutTransformControl>((x, _) => x.OnChildChanged());

            UseRenderTransformProperty.Changed
                .AddClassHandler<CalyLayoutTransformControl>((x, e) => x.OnUseRenderTransformPropertyChanged(e));
        }
        ...
        private void OnUseRenderTransformPropertyChanged(AvaloniaPropertyChangedEventArgs e)
        {
            ...
            var target = e.Sender as CalyLayoutTransformControl;
                ...
                if (shouldUseRenderTransform)
                {
                    _renderTransformChangedEvent = RenderTransformProperty.Changed
                            .Subscribe(
                                (x) =>
                                {
                                    if (x.Sender is CalyLayoutTransformControl target2)
                                    {
